...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <a class="btn btn-primary radius" data-title="添加教学日历" data-href="<c:url value="/admin/educationalDetail/calendarAdd.jsp" />" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加教学日历</a>
        </span>
        <span class="r">共有数据：<strong>${fn:length(calendars)}</strong> 条</span>
    </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <c:forEach var="calendar" items="${calendars}">
                <tr class="text-c">
                    <td>${calendar.cid }</td>
                    <td>${calendar.cname }</td>
                    <td>${calendar.createtime }</td>
                    <c:if test="${calendar.used == 1}">
                        <td><b>正在使用</b></td>
                    </c:if>
                    <c:if test="${calendar.used == 0}">
                        <td>无使用</td>
                    </c:if>
                    <td><c:if test="${calendar.used == 0}"><a href="<c:url value='/servlet/AdminCalendarServlet?method=use&cid=${calendar.cid}'/>">使用</a></c:if>
                        <c:if test="${calendar.used == 1}"><a href="<c:url value='/servlet/AdminCalendarServlet?method=stop&cid=${calendar.cid}'/>">停用</a></c:if>
                    <a href="<c:url value='/servlet/AdminCalendarServlet?method=delete&cid=${calendar.cid}'/>">删除</a>
