...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>教学安排 <span class="c-gray en">&gt;</span> 授课方法与手段 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
	    <span class="l">
	        <button onclick="removeIframe()" class="btn btn-primary radius">关闭选项卡</button></span>&nbsp;
        <a class="btn btn-primary radius" data-title="添加授课方法与手段" data-href="<c:url value="/admin/educationalDetail/teachingMeasureAdd.jsp" />" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加授课方法与手段</a>
        </span>
        <span class="r">共有数据：<strong>${fn:length(measures)}</strong> 条</span>
    </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <c:forEach var="measure" items="${requestScope.measures }">
                <tr class="text-c">
                    ...
                    <td>${measure.time }</td>
                    <c:if test="${measure.enable == true}">
                        <td><b>正在使用</b></td>
                    </c:if>
                    <c:if test="${measure.enable == false}">
                        <td>无使用</td>
                    </c:if>
                    <td><a href="<c:url value='/servlet/TeachingMeasureAdminServlet?type=useMeasure&measureId=${measure.measureId}'/> ">使用</a> <a href="<c:url value='/servlet/TeachingMeasureAdminServlet?type=delMeasure&measureId=${measure.measureId}'/> ">删除</a></td>
