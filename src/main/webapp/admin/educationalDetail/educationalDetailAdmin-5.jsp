...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <a class="btn btn-primary radius" data-title="添加教学重难点" data-href="<c:url value="/admin/educationalDetail/teachingHDPointsAdd.jsp" />" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加教学重难点</a>
        </span>
        <span class="r">共有数据：<strong>${fn:length(points)}</strong> 条</span>
    </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <tr class="text-c">
                <th width="40">ID</th>
                <th width="80">理论重难点</th>
                <th width="80">实验重难点</th>
            ...
            <c:forEach var="point" items="${requestScope.points }">
                <tr class="text-c">
                    <td>${point.pointId }</td>
                    <td>${point.theoryInfo }</td>
                    <td>${point.experimentInfo }</td>
                    <td>${point.time }</td>
                    <c:if test="${point.enable == true}">
                        <td><b>正在使用</b></td>
                    </c:if>
                    <c:if test="${point.enable == false}">
                        <td>无使用</td>
                    </c:if>
                    <td><a href="<c:url value='/servlet/TeachingHDPointsAdminServlet?type=usePoint&pointId=${point.pointId}'/> ">使用</a> <a href="<c:url value='/servlet/TeachingHDPointsAdminServlet?type=delPoint&pointId=${point.pointId}'/> ">删除</a></td>
