...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <a class="btn btn-primary radius" data-title="添加授课计划" data-href="<c:url value="/admin/educationalDetail/teachingPlanAdd.jsp" />" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加授课计划</a>
        </span>
        <span class="r">共有数据：<strong>${fn:length(plans)}</strong> 条</span>
    </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <c:forEach var="plan" items="${requestScope.plans }">
                <tr class="text-c">
                    ...
                    <td>${plan.planName }</td>
                    <td>${plan.time }</td>
                    <c:if test="${plan.enable == true}">
                        <td><b>正在使用</b></td>
                    </c:if>
                    <c:if test="${plan.enable == false}">
                        <td>无使用</td>
                    </c:if>
                    <td><a href="<c:url value='/servlet/TeachingArrangementAdminServlet?type=usePlan&planId=${plan.planId}'/> ">使用</a> <a href="<c:url value='/servlet/TeachingArrangementAdminServlet?type=delPlan&planId=${plan.planId}'/>">删除</a></td>
